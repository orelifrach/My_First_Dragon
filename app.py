    ...
    def input_action(self):
        ...
        while not is_valid:
            while not requested_action_str.isdigit():
                requested_action_str = input(f"The number of the requested action has to be a digit fronmt he options,\n{output_str}")
            print(int(requested_action_str))
            if not 0 < int(requested_action_str) <= len(actions_list):
                is_valid = False
                requested_action_str = input(f"The number of the requested action has to be a digit from the options,\n{output_str}")
            else:
                is_valid = True
